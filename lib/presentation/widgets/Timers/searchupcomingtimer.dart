...
class SearchtimerupScreen extends StatefulWidget {
   final int? index;
  const SearchtimerupScreen({this.index,super.key});

  @override
  State<SearchtimerupScreen> createState() => _SearchtimerupScreenState();
}

class _SearchtimerupScreenState extends State<SearchtimerupScreen> {
  ...
  void initState() {
   final res=Provider.of<Vehicleprovider>(context,listen:false);
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      DateTime startTime = dateFormat.parse(res.vlist?.upcomingbid?.data![widget.index!].starttime ??DateTime.now().toString());
      DateTime endTime = dateFormat.parse(res.vlist?.upcomingbid?.data![widget.index!].endtime??DateTime.now().toString());
